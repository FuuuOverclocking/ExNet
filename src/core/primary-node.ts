import { Dictionary, ElementType, LocalGroup, Node, ExNode, Port } from './types';
...
export class PrimaryNode<S, P extends object> implements LocalNode<S, P> {
    ...
    private setupPortsAndNodeRunConsolePrototype(): void {
        ...
        (this as any).ports = new LocalPortSet<P>(Port.Side.Outer, this.core, this);
        ...
        this.nodeRunConsolePrototype = new Proxy(this.rawNodeRunConsolePrototype, {
            get(target, prop) {
                ...
                if (It.isPortName(prop)) {
                    target.__RawNode__.ports.get(prop as keyof P, Port.Direction.Out);
    ...
    private listenForCorePortsStateChange(): void {
        ...
        const handler: Node.Event.CorePortsStateChangeHandler = (portName, action) => {
            ...
            if (action === Node.Event.CorePortsStateChangeAction.DetermineDirection) {
                node.portsState[portName as keyof P]!.direction = state.direction;
                this.ports[portName as keyof P].setDirection(state.direction, false);
                if (state.direction === Port.Direction.Out) {
                    this.addMethodToNodeRunConsole(portName as keyof P);
                }
    ...
    private addMethodToNodeRunConsole(portName: keyof P) {
        this.rawNodeRunConsolePrototype[portName as string] =
            this.rawNodeRunConsolePrototype[portName as string] ||
            function (this: Node.NodeRunConsole<PrimaryNode<S, P>>, data: any): void {
                this.__RawNode__.ports[portName].output(data, this.__ControlInfo__);
            };
    }
    ...
    public activate(data: any, controlInfo: Node.ControlInfo): void | Promise<void> {
        // Node is activated
        ...
        if (It.isPromise(asapResult)) {
            return asapResult.then(noop /* prevent the promise from giving Asap.cancelToken */);
        }
    }
    ...
    private throwError(nodeError: Node.NodeError): void {
        ...
        if (queue && queue.length) {
            for (const handler of queue) {
                const isCatched = handler.call(node.surfaceNode, false, nodeError);
                if (isCatched) return;
            }
        }
